...
use super::{material::WaterMaterialPropertype, shader::{WaterShader}, bind_group::IDWaterMaterialBindGroup, pipeline::WaterMaterialPipeline};

pub struct WaterMaterialUniformUpdate;
#[setup]
impl WaterMaterialUniformUpdate {
    #[system]
    pub fn tick(
        neshes: Query<GameObject, (&GlobalTransform, &MaterialID, &BuildinModelBind)>,
        materials: Query<GameObject, &WaterMaterialPropertype>,
        mut dynbuffer: ResMut<RenderDynUniformBuffer>,
    ) {
        println!("WaterMaterial Uniform TickUpdate");
        neshes
            .iter()
            .for_each(|(transform, mat_id, model)| match materials.get(mat_id.0) {
                Some(material) => {
                    println!("WaterMaterial >>>>>>>>>>>> ");
                    dynbuffer
                        .as_mut()
                        .set_uniform::<GlobalTransform>(&model.bind_offset, transform);
                    dynbuffer
                        .as_mut()
                        .set_uniform::<WaterMaterialPropertype>(&material.bind_offset, &material);
                }
                None => {
...
pub struct WaterMaterialFilter;
#[setup]
impl WaterMaterialFilter {
    #[system]
    pub fn tick(
        ...
        id_bind_group_default: Res<IDWaterMaterialBindGroup>,
        mut pipelines: ResMut<WaterMaterialPipeline>,
        mut pipeline_pool: ResMut<SingleRenderObjectPipelinePool>,
    ) {
        let id_bind_group_default = id_bind_group_default.0;
        println!("WaterMaterial Filter");
        query_camera
            .iter()
...
fn collect_opaque_normal_depth(
    ...
    normals: &Query<GameObject, &AttributeNormal>,
    indices: &Query<GameObject, &AttributeIndices>,
    pipelines: &mut WaterMaterialPipeline,
    device: & RenderDevice,
    shader: & WaterShader,
