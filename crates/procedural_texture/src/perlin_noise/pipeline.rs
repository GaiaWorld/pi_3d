...
use super::{material::PerlinNoiseMaterialPropertype, shader::PerlinNoiseShader};

pub struct PerlinNoiseMaterialPipeline {
    pub map: XHashMap<u128, DefaultKey>,
}
impl Default for PerlinNoiseMaterialPipeline {
    fn default() -> Self {
        Self {
...
impl PerlinNoiseMaterialPipeline {
    pub fn build(
        &mut self,
        device: &RenderDevice,
        ...
        match self.map.get(&key) {
            None => {
                ...
                let bind_group_1_layout = BindGroupLayout::from(device.create_bind_group_layout(
                    &wgpu::BindGroupLayoutDescriptor {
                        label: Some("PerlinNoise"),
                        entries: &[BuildinModelBind::ENTRY, PerlinNoiseMaterialPropertype::ENTRY],
                    },
                ));
