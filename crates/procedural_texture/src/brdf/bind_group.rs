use pi_ecs::{
    prelude::{query, Query, Res, ResMut},
    query::{Changed, Or, WithOut, Write},
};
...
use pi_scene_context::{
    materials::{
        bind_group::{RenderBindGroup, RenderBindGroupPool},
...
use super::{
    define::{UnlitMaterialDefines, UnlitMaterialMode},
    ...
    pub fn layout(device: &RenderDevice) -> BindGroupLayout {
        BindGroupLayout::from(
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                ...
                entries: &[BuildinModelBind::ENTRY, UnlitMaterialPropertype::ENTRY],
            }),
        )
    }
...
impl UnlitMaterialTextureBindGroup {
    pub fn label(mode: UnlitMaterialMode) -> &'static str {
        "UnlitMaterialTextureBindGroup"
    }

    pub fn slot(mode: UnlitMaterialMode) -> u8 {
        2
    }
    ...
    pub fn layout(mode: UnlitMaterialMode, device: &RenderDevice) -> BindGroupLayout {
...
impl SysUnlitMaterialBindGroupUpdate {
    ...
    pub fn tick(
        ...
        if dynbuffer_flag.0 {
            UnlitMaterialBindGroup::bind_group(
                &device,
                bindgroups.get_mut(unlit_bindgroup.value.unwrap()).unwrap(),
                &dynbuffer,
            );
        }
    }
...
impl SysUnlitMaterialTextureBindGroupUpdate {
    ...
    pub fn tick(
        ...
        mut items: Query<
            GameObject,
            (
                &UnlitMaterialDefines,
                &MainTextureRes,
                &MainTextureSampler,
                Write<UnlitMaterialTextureBindGroup>,
            ),
            Or<(
                Changed<UnlitMaterialDefines>,
                Changed<MainTextureSampler>,
                Changed<MainTextureRes>,
            )>,
        >,
        ...
        items
            .iter_mut()
            .for_each(|(define, tex2d, sampler, mut texbindgroupwrite)| {
                ...
                match unlit_bindgroup.texture_map.get(&deffinemode) {
                    ...
                    None => {
                        let group = bindgroups.creat(
                            &device,
                            UnlitMaterialTextureBindGroup::layout(deffinemode, &device),
                            UnlitMaterialTextureBindGroup::slot(deffinemode) as u32,
                        );
                        let texbindgroup = UnlitMaterialTextureBindGroup(group);
                        texbindgroup.bind_group(
                            deffinemode,
