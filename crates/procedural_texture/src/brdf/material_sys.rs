...
use pi_scene_context::{
    ...
    materials::{bind_group::RenderBindGroupPool, material::MaterialID},
    ...
    renderers::{
        ...
        render_mode::{ERenderMode, RenderMode},
        render_object::{
            RenderObjectBindGroup, RenderObjectID, RenderObjectIndices, RenderObjectMetaOpaque,
            RenderObjectMetaTransparent, RenderObjectVertice,
        },
    ...
    transforms::{dirty::DirtyGlobalTransform, transform_node::GlobalTransform},
    vertex_data::{
        ...
        uv::{AttributeUV, IDAttributeUV},
    },
};

use super::{
    bind_group::{UnlitMaterialBindGroup, UnlitMaterialTextureBindGroup},
    define::UnlitMaterialDefines,
    material::UnlitMaterialPropertype,
    pipeline::UnlitMaterialPipeline,
    shader::UnlitShaderPool,
};
...
impl UnlitModelUniformUpdate {
    ...
    pub fn tick(
        meshes: Query<
            GameObject,
            (&GlobalTransform, &MaterialID, &BuildinModelBind),
            With<DirtyGlobalTransform>,
        >,
...
impl UnlitMaterialUniformUpdate {
    ...
    pub fn tick(
        ...
        materials.iter().for_each(|(material)| {
            dynbuffer
                .as_mut()
                .set_uniform::<UnlitMaterialPropertype>(&material.bind_offset, &material);
        });
    }
}
    ...
    pub fn tick(
        mut items: Query<
            ...
            Or<(
                Changed<MaterialID>,
                Changed<RenderBlend>,
                Changed<RenderDepthAndStencil>,
                Changed<PrimitiveState>,
            )>,
        >,
        ...
        items.iter_mut().for_each(
            |(matid, blend, depth_stencil, primitive, mut pipelinekey)| {
...
impl UnlitMaterialFilter {
    ...
    pub fn tick(
        ...
        materials: Query<
            GameObject,
            (
                &UnlitMaterialPropertype,
                &UnlitMaterialBindGroup,
        ...
        query_camera
            .iter()
            .for_each(|(renderid, sceneid, layermask, camerapos)| {
            ...
            if camera_sceneid == sceneid.0 && layermask.include(&layer) {
                ...
                if let Some((mat, valuebindgroup, texbindgroup)) = materials.get(matid.0) {
                    match (
                        ...
                        normals.get(normal.0),
                        indices.get(indice.0),
                        uvs.get(uv.0),
                    ) {
