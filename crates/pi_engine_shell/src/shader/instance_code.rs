use pi_render::renderer::{attributes::{VertexAttribute, EVertexDataKind}, buildin_var::{ShaderVarVertices, ShaderVarUniform}};
...
impl InstanceState {
    ...
    pub fn attributes(state: u32) -> Vec<VertexAttribute> {
        ...
        if (state & Self::INSTANCE_CUSTOM_VEC4_C) == Self::INSTANCE_CUSTOM_VEC4_C {
            result.push(VertexAttribute { kind: EVertexDataKind::InsCustomVec4C, format: wgpu::VertexFormat::Float32x4 });
        }
...
impl Default for EVerticeExtendCode {
    fn default() -> Self {
        Self(Self::NONE)
    }
}
