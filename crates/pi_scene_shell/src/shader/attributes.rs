...
impl CustomVertexAttribute {
    pub fn new(
        ...
        Self { key, vscode, vtype, foruniform }
    }
    ...
    pub fn vs_varying_code(&self, index: u32) -> &str {
        self.vscode.as_str()
    }
    pub fn fs_varying_code(&self, index: u32) -> &str {
        self.vscode.as_str()
    }
...
impl EVertexAttribute {
    ...
    pub fn define_code(&self, location: u32) -> String {
    ...
    pub fn var_code_for_varying(&self, inout: &str, extend_varying_startidx: u32, meta: &ShaderEffectMeta) -> Option<String> {
        match self {
            EVertexAttribute::Buildin(_val, _format) => None,
            EVertexAttribute::Custom(val) => match &val.foruniform {
                Some(key) => {
                    if meta.uniforms.query_instance(key) {
                        let mut result = String::from("layout(location = ");
                        result += extend_varying_startidx.to_string().as_str();
                        result += ") ";
                        result += inout;
                        result += crate::prelude::S_SPACE;
                        result += self.kind().as_str();
        ...
        match self {
            EVertexAttribute::Buildin(val, _) => {
                ...
                match val {
                    ...
                    EBuildinVertexAtribute::MatIdxs => {
                        result += Self::matidx().as_str();
                    },
                    _ => {},
                }
            },
    ...
    pub fn matidx() -> String {
        ...
        result += " = ";
        result += ShaderVarVertices::INS_MAT_IDX;
        result += ";\r\n";
