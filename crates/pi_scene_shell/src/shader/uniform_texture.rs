...
fn texture_code(slotname: &str, bindname: &str, tex_sampler_type: &wgpu::TextureSampleType, dimension: wgpu::TextureViewDimension, texidx: usize, _engineopt: &EngineCustomPlugins) -> String {
    let mut uv = String::from("uvAtlas(uv, ");
    uv += "matParam.Atlas";
    uv += &texidx.to_string();
    // uv += slotname;
    ...
    match tex_sampler_type {
        wgpu::TextureSampleType::Float { .. } => match dimension {
            ...
            wgpu::TextureViewDimension::D2          => String::from("vec4 Get") + slotname + "(const vec2 uv, const MatParam matParam){ return texture(sampler2D(" + bindname + ", sampler" + bindname + "), " + uvatlas +"); }\n",
            wgpu::TextureViewDimension::D2Array     => String::from("vec4 Get") + slotname + "(const vec2 uv, const MatParam matParam){ return texture(sampler2DArray(" + bindname + ", sampler" + bindname + "), vec3(" + uvatlas +", " + &coord + ")); }\n",
        ...
        wgpu::TextureSampleType::Depth => String::from("vec4 Get") + slotname + "(const vec2 uv, const MatParam matParam){ return texture(sampler2D(" + bindname + ", sampler" + bindname + "), " + uvatlas +"); }\n",
        wgpu::TextureSampleType::Sint => String::from("ivec4 Get") + slotname + "(const vec2 uv, const MatParam matParam){ return texture(sampler2D(" + bindname + ", sampler" + bindname + "), " + uvatlas +"); }\n",
        wgpu::TextureSampleType::Uint => String::from("uvec4 Get") + slotname + "(const vec2 uv, const MatParam matParam){ return texture(sampler2D(" + bindname + ", sampler" + bindname + "), " + uvatlas +"); }\n",
    }
}
