...
use crate::{pass::EPassTag, animation::command};
...
impl ActionMaterial {
    pub fn create_material(
        ...
        commands
            .insert(AssetKeyShaderEffect(key))
            .insert(MaterialUsedList::default())
            .insert(passtag)
            .insert(UniformTextureWithSamplerParams::default())
            .insert(DirtyMaterialUsedList);
    }
    
    pub fn use_material(
        commands: &mut EntityCommands,
        user: Entity,
        uselist: &mut MaterialUsedList,
    ) {
        uselist.0.insert(user, user);
        commands.insert(DirtyMaterialUsedList);
    }

    pub fn unuse_material(
        user: Entity,
        uselist: &mut MaterialUsedList,
    ) {
        uselist.0.remove(&user);
    }
}
