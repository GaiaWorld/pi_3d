...
#[cfg(feature = "use_bevy")]
        ...
        app
        .configure_set(Update, StageSkeleton::SkinCreate    /* .run_if(runif_3d) */.in_set(FrameDataPrepare).after(StageModel::_InitMesh))
        .configure_set(Update, StageSkeleton::_SkinCreate   /* .run_if(runif_3d) */.in_set(FrameDataPrepare).after(StageSkeleton::SkinCreate).before(StageTransform::TransformCommand))
        .configure_set(Update, StageSkeleton::Command       /* .run_if(runif_3d) */.in_set(FrameDataPrepare).after(StageSkeleton::_SkinCreate).before(ERunStageChap::Uniform))
        .configure_set(Update, StageSkeleton::Calc          /* .run_if(runif_3d) */.in_set(FrameDataPrepare).after(StageSkeleton::Command).after(StageTransform::TransformCalcMatrix).before(ERunStageChap::Uniform))
        ;
