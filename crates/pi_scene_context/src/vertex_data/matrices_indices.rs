...
struct CommandListBufferMatricesIndices {
...
impl SysCommand {
    #[system]
    pub fn cmd(
        mut cmds: ResMut<CommandListBufferMatricesIndices>,
        mut items: Query<GameObject, Write<AssetKeyBufferMatricesIndices>>,
    ) {
            ...
            match cmd {
                ECommand::Use(entity, key) => {
                    if let Some(mut item) = items.get_mut(entity) {
                        item.write(AssetKeyBufferMatricesIndices(key.clone()));
                    }
                },
            }
        });
...
pub struct AssetKeyBufferMatricesIndices(pub KeyVertexBuffer);

#[derive(Deref, DerefMut)]
pub struct AssetResBufferMatricesIndices(pub Handle<VertexBuffer>);
impl From<Handle<VertexBuffer>> for AssetResBufferMatricesIndices {
    fn from(value: Handle<VertexBuffer>) -> Self {
        Self(value)
    }
}
impl AssetResBufferMatricesIndices {
    pub const NUMBER_BYTES: wgpu::BufferAddress = 4;
    pub const NUMBER_COUNT: wgpu::BufferAddress = 3;
    pub const OFFSET: u32 = 0 * 4;
}
impl TVertexBufferMeta for AssetResBufferMatricesIndices {
...
impl TAttributeMeta for AttributeMatricesIndices {
...
pub trait InterfaceBufferMatricesIndices {
...
impl InterfaceBufferMatricesIndices for Engine {
    fn use_vertex_data_normal(
        & self,
        entity: ObjectID,
        key: KeyVertexBuffer,
    ) -> &Self {
        let commands = self.world().get_resource_mut::<CommandListBufferMatricesIndices>().unwrap();
...
pub struct PluginBufferMatricesIndices;
impl Plugin for PluginBufferMatricesIndices {
    fn init(
        &mut self,
        ...
        PluginAssetSyncLoad::<KeyVertexBuffer, AssetKeyBufferMatricesIndices, VertexBuffer, AssetResBufferMatricesIndices>::new(false, 60 * 1024 * 1024, 60 * 1000).init(engine, stages);

        let world = engine.world_mut();
        world.insert_resource(CommandListBufferMatricesIndices::default());
