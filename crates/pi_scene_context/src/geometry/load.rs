...
use super::{base::*, AssetDescVBSlots, AssetResVBSlots, LoadedKeyVBSlots};
    ...
    pub fn sys_vertex_buffer_loaded(
        ...
        data0.drain().for_each(|(key, range)| {
            geoloader.loader_vertices.loaded(&key, &range).drain(..).for_each(|((id, slot), data)| {
                if let Ok((keys, mut loadedkeys, mut reslist, _, _, _, mut flag)) = geometries.get_mut(id) {
        ...
        data1.drain().for_each(|(key, range)| {
            ...
            geoloader.loader_indices.loaded(&key, &range).drain(..).for_each(|(id, data)| {
                if let Ok((_, _, _, mut buffer, mut loadedkey, desc, mut flag)) = geometries.get_mut(id) {
