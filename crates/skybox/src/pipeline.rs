...
use super::{material::SkyboxMaterialPropertype, shader::SkyboxShader};

pub struct SkyboxMaterialPipeline {
    pub map: XHashMap<u128, DefaultKey>,
}
impl Default for SkyboxMaterialPipeline {
    fn default() -> Self {
        Self {
        ...
        match self.map.get(&key) {
            None => {
                ...
                let bind_group_1_layout = BindGroupLayout::from(device.create_bind_group_layout(
                    &wgpu::BindGroupLayoutDescriptor {
                        label: Some("Skybox"),
                        entries: &[BuildinModelBind::ENTRY, SkyboxMaterialPropertype::ENTRY],
                    },
                ));
