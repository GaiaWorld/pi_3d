...
fn setup(
    ...
    let mut config = IParticleSystemConfig {
        ...
        size_over_lifetime: Some(ParamInfo::ThreeParamInfo(
    ...
    fn add_frame_event<T: Event>(&mut self) -> &mut Self {
        if !self.world.contains_resource::<Events<T>>() {
            self.init_resource::<Events<T>>()
                .add_systems(StageD3, Events::<T>::update_system);
        }
        self
    }
}
...
pub fn main() {
    ...
    app.add_systems(StageD3, sys_demo_particle.in_set(StageModel::RenderMatrix));
