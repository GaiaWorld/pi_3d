...
impl PluginImageCopy {
    ...
    //         let id_geo = commands.insert(());
    //         let attrs = QuadBuilder::attrs_meta();
            
    //         let plane = commands.insert(()); actions.transform.tree.push(OpsTransformNodeParent::ops(plane, scene));
    ...
    //         let copycamera = commands.insert(()); actions.transform.tree.push(OpsTransformNodeParent::ops(copycamera, scene));
    ...
    //         let copy_renderer = commands.insert(()); actions.renderer.create.push(OpsRendererCreate::ops(copy_renderer, String::from("ImageCopy") + copy_renderer.data().as_ffi().to_string().as_str(), copycamera, PassTag::PASS_TAG_01, false));
