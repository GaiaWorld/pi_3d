...
fn setup(
    ...
    let tes_size = 20;
    fps.frame_ms = 30;
    actions.scene.brdf.push( OpsSceneBRDF::ops(commands.insert(()), Atom::new("url"), true))
    

    // let demopass = DemoScene::new(&commands, &mut actions, &mut animegroupres, 
    //     &mut assets.0, &assets.1, &assets.2, &assets.3,
    //     4., 0.7, (0., 0., -10.), false
    // );
    // let (scene, camera01) = (demopass.scene, demopass.camera);

    // let (copyrenderer, copyrendercamera) = copy::PluginImageCopy::toscreen(&commands, &mut actions, scene,  demopass.transparent_renderer, demopass.transparent_target);
    ...
    // let root = commands.insert(()); actions.transform.tree.push(OpsTransformNodeParent::ops(root, scene));
    ...
    // let source = base::DemoScene::mesh(& commands, scene, scene, &mut actions,  vertices, indices, state);

    // let idmat = defaultmat.0;
    // actions.material.usemat.push(OpsMaterialUse::ops(source, idmat, DemoScene::PASS_OPAQUE));
    
    // // let key_group = pi_atom::Atom::from("key_group");
    // let id_group = commands.insert(());
