...
use pi_3d::{systems::{transform_node_sys::{LocalMatrixCacl, WorldMatrixCacl, LocalRotationMatrixCacl}, camera_sys::{TargetCameraEffectLocalRotation, TargetCameraViewMatrixCacl, FreeCameraProjectionCacl, CameraTransformMatricCacl}}, object::GameObject, engine::Engine};
use pi_async::{rt::{AsyncRuntimeBuilder, AsyncRuntime}, prelude::{WorkerRuntime, Mutex}};
use pi_ecs::{prelude::{World, IntoSystem, System, Query, ArchetypeId, StageBuilder, Setup, SingleDispatcher, Res, ResMut}, entity::Id};
...
pub fn create_engine(
	...
	let mut result: Share<ShareRwLock<Option<(Engine, SingleDispatcher<WorkerRuntime>)>>> = Share::new(ShareRwLock::new(None));
	let result1 = result.clone();
    
	let _ = runtime.spawn(runtime.alloc(), async move {
		let world = &mut world1;
		let options = RenderOptions::default();
		let render_stages = init_render::<Option<ShareTargetView>>(world, options, win1.clone(), rt.clone()).await;
