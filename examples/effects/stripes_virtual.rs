...
fn setup(
    ...
    ActionMaterial::regist_material_meta(&matcmds.3, &mut matcmds.4, KeyShaderMeta::from(StripesVirtualShader::KEY), StripesVirtualShader::create(&matcmds.6));
    ...
    geometrycreate.push((source, id_geo, 
    ...
    matcmds.1.push(OpsMaterialCreate::ops(idmat, StripesVirtualShader::KEY, EPassTag::Transparent));
    matcmds.2.push(
        OpsUniformByName::Float(
            idmat, 
            String::from(StripesVirtualShader::KEY_SPEED), 
            1.,
            true
        )
    );
    matcmds.2.push(
        OpsUniformByName::Float(
            idmat, 
            String::from(StripesVirtualShader::KEY_STEP), 
            0.1,
            true
        )
    );
