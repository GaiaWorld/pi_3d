...
impl Plugin for PluginTest {
    ...
    fn setup(
        ...
        renderercmds.modify.push(OpsRendererCommand::DepthClear(demopass.opaque_renderer, RenderDepthClear(1.)));
        ...
        scenecmds.shadowmap.push(OpsSceneShadowMap::ops(scene, demopass.shadowtarget));
        {
            ...
            let direction =  (0., -5., 0.);
            let color = (1., 0.2, 0.2);
            let light = light::DemoLight::spotlight(&mut commands, scene, scene, &mut transformcmds, &mut lightingcmds, &mut meshcmds.layermask, position, direction, color, 0xFFFFFFFF);
            lights.push(light);

            {
                ...
                shadowcmds.param.push(OpsShadowGeneratorParam::ShadowMinz(shadow, 0.0));
            }
        }
        // {
        //     let position = (0., 0., 0.);
        ...
        for i in 0..(tes_size * ttt) {
            for j in 0..(tes_size * ttt) {
                for k in 0..(tes_size * ttt) {
                    ...
                    transformcmds.localpos.push(OpsTransformNodeLocalPosition::ops(cube, (i as f32 - half) / tttf, (k as f32 - half * 0.5) / tttf + 5., (j as f32 - half) / tttf));
