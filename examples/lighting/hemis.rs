...
impl Plugin for PluginTest {
    ...
    fn setup(
        ...
        for i in 0..light_colors.len() {
            let light = commands.spawn_empty().id(); transformcmds.tree.push(OpsTransformNodeParent::ops(light, lightroot));
            meshcmds.layermask.push(OpsLayerMask::ops(light, 0xFFFFFFFF));
            lightingcmds.create.push(OpsLightCreate::ops(scene, light, ELightType::Point, ColorFormat::Rgba16Float, DepthStencilFormat::Depth32Float, ShadowAtlasSize::DEFAULT, ShadowAtlasSize::DEFAULT));
            let color = &light_colors[i]; let pos = &light_position[i];
            lightingcmds.color.push(OpsLightColor::ops(light, color[0], color[1], color[2]));
            transformcmds.localpos.push(OpsTransformNodeLocalPosition::ops(light, pos[0], pos[1], pos[2]));
            lights.push(light);
        }
    ...
    geometrycmd.create.push(OpsGeomeryCreate::ops(source, id_geo, attrs.clone(), Some(CubeBuilder::indices_meta())));
    ...
    meshcmds.depth_compare.push(OpsDepthCompare::ops(source, CompareFunction::Greater));
    lights.iter().for_each(|light| {
        abstructmeshcmds.force_spot_light.push(OpsMeshForceSpotLighting::ops(source, *light, true));
    });
